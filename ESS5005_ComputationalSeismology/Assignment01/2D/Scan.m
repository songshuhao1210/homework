function [flag_inf] = Scan(flag_eps,type_c,flag_bound,dx,folder_fig)
    %% parameter settings
    % grid paras
    WE = [-1000,1000];              % horizontal range left-right   /m
    Z =  1000;                      % depth range  /m
    ...
    elseif type_c == 2
        value_c = [344,90,1430;0,200,500]; %   2--layers; value_c(1,:)=velocity , value_c(2,:) = start depth
    elseif type_c == 3
        value_c = [0.2,0.4,1000];    % 3--2D anisotropic; c(z,x) = value_c(1)*xx+value_v(2)*zz+valuec_(3)
    else
    ...
    speed = 50;                            % speed of gif
        ...
        % source parameters
    source_paras{1} = 1;                    % the number of sources
    ...
    source_paras{4} = [100,100;1000,1000];           % the source location (xs,zs) /km
    source_paras{5} = [3;3];                % basic function of source. 1--delta function; 2--unistep; 3--single frequency source
    source_paras{6} = [1;1];               % extra information, i.e. f0
